...
--------------------------------
  1     人体关键点（骨架）开关
  2     分析框与 编号/划水/速度 标签开关（编号见下面「运动员编号」）
...
两组都默认关掉「检测」（框线压在入水的手臂上会挡泳姿），跟随还默认关掉关键点，
只留分析标签与网格 —— 速度与划数还要读。「检测」是框本身，「分析」是那行
文字标签，两者分开控制。跟随视角里标签钉在左上角固定位置，不跟着检测框上下抖；
全景则贴各自的框沿，因为同一条道可能两个人并列游。
**正在跟随的那个人，在全景里骨架会亮一圈白光并加粗**，所以即使不开检测框也一眼
看得出锁定的是谁（他还会被画在最上层，不会被别人的骨架压住）。
...
跟随那一组还多一个「自动接管」（默认开），作用是让跟随画面在有人游的时候永不空着：
刚切到「全景 + 个人」还没点过人时立刻跟上，正在跟的人游出画面或丢检超过 1.5 秒
就换人，落点都是「近 5 秒平均速度最快」的那个人，也就是统计栏里显示的那一位。
用平均速度而不是瞬时速度，是为了让读数与镜头都不闪。池里没人时它就等着，
有人下水约 3 秒后自动跟上。不想让它自己挑人就取消勾选（勾着时点空水面会被立刻
接管回来）。手工点人随时接管回来。

统计栏只报「此刻在场的人」：在场人数、平均速度、平均划频、平均每划距离、
近 5 秒平均速度最快者及其编号、八条泳道各自的当前占用。
个人栏标题就是他的编号，内容为划水次数、当前/平均/峰值速度、划频 spm、
在场时长、累计里程、每划距离、划水指数（均速 × 每划距离，同样速度下越大越省力）。

运动员编号
----------
界面上一律是「x 道 y 号」：网页写作「7 道 1 号」，窗口与落盘 mp4 因为字体没有中文
写作 L7-1（同一个号；站在池岸、不在任何泳道里的人显示「池岸」/ SIDE）。

道内的号是**占位式**，从 1 开始：
  * 有人退出腾出的号会被下一个入场的人补上（5 号退了，下一人就是 5 号）
  * 号全被占满才往后加（10 人占满，下一人是 11 号）
  * 短暂被水花挡住不会换号（保留约 1.5 秒），压着分道绳游也不会在两道之间跳
  * 同一人从一条道游到另一条道，会还掉旧道的号、在新道拿一个号

所有统计（速度、划水、里程、排名）都按程序内部的唯一编号算，与显示的号无关，
所以换道、补号都不会把两个人的数据串在一起。
