...
rem Called (not executed) by run_preview.bat / run_analyse.bat, so it must NOT
rem use setlocal: PATH and EXE have to survive back into the caller.
rem ASCII only on purpose: cmd.exe parses .bat with the system ANSI codepage.
rem
rem   call cpp\env.bat || goto :fail
rem
...
if not exist "%EXE%" (
  echo [error] %EXE% missing - build it first: double-click build.bat
  exit /b 1
)
...
:no_model
echo [error] cpp\models\detect.onnx or pose.onnx missing - run build.bat
exit /b 1
