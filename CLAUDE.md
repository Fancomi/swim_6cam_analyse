...
> ⚠ 这四组是 **train-3 检测权重**的数字。检测权重已升级到上游 **train-4**
> （`weights/yolo_swim_detect.pt`，train-4 召回更高：六路 `--cam-dir` 人次
> 25078→27148、track 91→139，H800 实测），**四行全部作废**，须在**基线机
> RTX 4080 Laptop** 上重导 ONNX 后 `bash scripts/test.sh --baseline` 重新采集
> 填回（H800 与 RTX4080 数值本就不同 —— 同一 train-3 六路划水 H800 得 901、
> RTX4080 得 883，故不能拿 H800 数字直接顶进这张 RTX4080 口径表）。
