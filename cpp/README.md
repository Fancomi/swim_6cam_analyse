...
### 丢检占位（`--ghost SEC`，默认 1 s）

detect 会偶发漏检（实测同一个人一秒内可丢两三帧），画面上表现为框与骨架闪断。
`--ghost` 让丢检的 track 把**上一次的观测原样停在原地** —— 框、关键点都不动 ——
最多停 SEC 秒，之后不再画（`--ghost 0` 关掉，退回旧行为）。

参数是**秒不是帧**：漏检是时间现象，30/60 fps 两种源不该配两个值；`Pipeline`
构造时按源帧率折算成帧数交给 `Tracker`。占位不延长 track 的寿命：ID 保留期仍由
`--track-max-lost`（帧）决定，超了 track 直接销毁，所以实际占位时长是
`min(--ghost, --track-max-lost/fps)`，默认 30 帧 / 1 s 恰好同量级。
...
因此 `--ghost` 是**纯显示层特性**：同一段视频 `--ghost 0` 与 `--ghost 1` 的
`--json` 与 `--dump` **逐字节相同**（300 帧实测，2721 人次 / 18 track / 34 次划水
两边一致），只有渲染与 `[Summary]` 多出一行占位统计：

```
[Summary] ghost 占位 365 个框 (13.41% of 人次), 每框最长 1.00 s
```

这行顺带就是**漏检率的量**（13.4% ≈ 每人每秒丢 2~4 帧，与现场观察一致），
`--show-fps` 那行的 `当前 9(+2) 人` 里 `(+2)` 是本帧占位数 —— 它持续偏高说明
detect 在漏检，与网络掉帧（看 `| 到帧` 那六个数）是两件不同的事。
