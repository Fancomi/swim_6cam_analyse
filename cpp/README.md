...
| 分析 | `2` | `--no-boxes` | 检测框 + `ID:n S:n v m/s` 标签（丢检占位框同样画法） |
...
**自动接管**（跟随组第五个勾选框，默认开）：目标离场满 1.5 秒后自动切到
`all.hotid`，即「近 5 秒均速最快」的在场者 —— 与概览里那一行是同一个数。
判据用均速而非瞬时速度：瞬时每帧换人，读数与镜头都会乱闪。1.5 秒的宽限是给
ghost 占位留的余地（短暂丢检很常见，一丢就跳会让画面乱蹦）；宽限按 `/stats` 的
流内秒数 `t` 计，暂停或掉帧时与墙钟分叉也不会误判。手工点人立即重置计时。

### 统计口径

**全场只报「此刻在场的真检出者」**：在场人数、平均速度、平均划频、平均每划距离、
**近 5 秒均速最快者及其 ID**、分道占用。
...
（里程还没积起来，一两次关键点抖动就能排到第一）。窗口长度由 `kRecentSec` 一处定，
随 `/stats` 的 `all.hotsec` 下发给前端做标题，改常量不必改前端。

个人面板是划水次数 / 当前与平均速度 / 划频 / 所在泳道 / 在场时长 / 累计里程 /
峰值速度 / 每划距离 / **划水指数**（`vavg × dps`，泳界通用的效率指标：同样速度下越大
越省力）。均速一律用「里程 ÷ 在场时长」而非瞬时速度的平均 —— 后者会被丢检期间的
