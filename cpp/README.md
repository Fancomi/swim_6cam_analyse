...
计时里只剩 0.02 ms/帧。`--preview` 与 `--web` 走同一次 D2H，因此三者同开不会拷三遍。
...
| | `scripts\preview.bat`（开窗口） | `scripts\web.bat`（开浏览器） | `scripts\analyse.bat`（写文件） |
| --- | --- | --- | --- |
| 省略 / `canvas` | 实时看已拼画布 | 看板看已拼画布 | 批处理已拼画布 → json |
| `6cam` | 实时看六路现拼 | 看板看六路现拼 | 批处理六路现拼 → json |
| 视频文件（可拖） | 实时看那段视频 | 看板看那段视频 | 批处理那段视频 |
| 目录（可拖） | 实时看那批六路片段 | 看板看那批片段 | 批处理那批六路片段 |
| 相机清单文件 | 实时看六路 ZCam 现拼 | 看板看六路 ZCam 现拼 | 批处理（流没有结尾，需配 `--max-frames`） |
| `rtsp://…` | 实时看单路直播画布 | 看板看单路直播画布 | 报错（流没有结尾） |

三者跑的是同一套算法、同一份 exe，其余参数原样透传
（`scripts\analyse.bat 6cam --out o.mp4 --max-frames 300`）。命令行解析、源解析与
前置检查都在 `scripts\env.bat` 里做一次，三个 launcher 各只剩十几行。
默认源用 `SWIM_CANVAS` / `SWIM_CAM_DIR` 覆盖。看板的细节见「浏览器看板」。
...
# 浏览器看板（默认 http://127.0.0.1:8080/，自动开浏览器；见「浏览器看板」）
./build/swim_analyse --input data/xxx.mp4 --models cpp/models --web --show-fps
...
# 机位倒挂：整块画布转 180°（尺寸不变，零额外开销，见「画面定向」）
./build/swim_analyse --cam-dir configs/cameras.txt --models cpp/models --rot180 --preview
...
detect 会偶发漏检（实测同一个人一秒内可丢两三帧），画面上表现为框与骨架闪断。
`--ghost` 让丢检的 track 把**上一次的观测原样停在原地** —— 框、关键点都不动 ——
最多停 N 帧，之后不再画（`--ghost 0` 关掉，退回旧行为）。

参数用**帧**而不是秒，与 `--track-max-lost` 同单位：漏检本就是"连续几帧没检出"的
直接计数，而占位框是**冻结**的 —— 停太久会明显落在人后面（1.5 m/s 的人 30 帧走
1.5 m，几乎一个身位），所以默认只给 5 帧（30 fps 下约 0.17 s），刚好盖住实测的
"一秒丢两三帧"而不产生可见滞后。占位不延长 track 的寿命：ID 保留期仍由
`--track-max-lost` 决定，超了 track 直接销毁，所以实际占位时长是
`min(--ghost, --track-max-lost)`。

**能力边界刻意切在"只进画面"**：占位框在 `Person::lost` 上带着已丢帧数（真检出为
0），于是

| 去处 | 占位框 |
| --- | --- |
| `--preview` / `--out` 渲染 | **画**，与真检出**完全同一画法**（同色、同线宽、同标签） |
| `[Summary]` 人次 / track 数 / 划水 | 不计 |
| `--json` / `--dump` CSV | 不写 |
| 划水与速度信号（`MetricsTracker`） | 不喂 |

渲染不做区分是刻意的：这个特性的目的就是"人一直在"，而某一帧检没检出属于内部
状态；把占位框画淡或加后缀，等于把被消掉的闪烁换成另一种闪烁。要看漏检有多少，
读 `[Summary]` 那行占位统计与 `--show-fps` 的 `(+N)`，两处都是准确计数。

不喂信号则是硬约束而非保守：信号位置按**观测次数**计（见 `metrics.h`），插一个
"原地不动"的样本会把划水波谷抹平、速度压到 0，且与 Python 参考的逐值对照当场失效。
反过来，标签上的 `S:` 与速度取的是该 track 的**当前读数**，所以占位期间显示的是
最后一次真实值，不会跳成 0。

因此 `--ghost` 是**纯显示层特性**：同一段视频 `--ghost 0` 与开着 ghost 的
`--json` 与 `--dump` **逐字节相同**（默认数据集 3000 帧实测，两边同为 24107 人次 /
63 track / 875 次划水），只有渲染与 `[Summary]` 多出一行占位统计：

```
[Summary] ghost 占位 1086 个框 (4.50% of 人次), 每框最长 5 帧
```

这行顺带就是**漏检率的量**，`--show-fps` 那行的 `当前 9(+2) 人` 里 `(+2)` 是本帧
占位数 —— 它持续偏高说明 detect 在漏检，与网络掉帧（看 `| 到帧` 那六个数）是两件
不同的事。注意这个百分比随 N 增大而增（同一段 3000 帧：5 帧时 4.50%，30 帧时
12.23%），因为同一次漏检被多画几帧就多记几个框；比较漏检率要在同一个 N 下比。

`--ghost` **Python 侧没有对应参数**，也不需要有：Python 是离线批处理，Stage3 的
`interpolate_keypoints` 在首末可信帧之间做线性插值（插值点置信度置 0），本就不会
闪断；在线路不能等全序列，才需要这条只向前停留的占位。两侧的数值口径因此不变，
`同步契约` 第 3 条（同义参数同名）不涉及本参数。
...
| `--web` | 推浏览器看板（见「浏览器看板」） | 17.3 ms/帧（58 fps） |
...
| 分析 | `2` | `--no-boxes` | 检测框 + `L7-1 S:n v m/s` 标签（编号见「两套编号」；丢检占位框同样画法） |
| 米制网格 | `3` | `--grid` | 粗线=泳道分割线（每 2.5 m，落在分道绳上），细线均分 0.5 m |

看板（`--web`）有同名的三个复选框，但**是浏览器用 `<canvas>` 另画的**，不受这里的
热键与启动开关影响 —— C++ 推给浏览器的始终是未标注画面。口径靠 `/meta` 保持一致，
见「浏览器看板」。

米制网格**不读标定**：它只用 `--ppm`（默认 100 px/m）和画面尺寸换算，用途是
目视量距离、核对速度口径（速度 = 框中心位移 / ppm）。所以画布与六路现拼两条路
画出来的是同一套刻度。**刻度以泳道为基准**：粗线每 `kLanePitchM`=2.5 m 一条
（就是那 9 排红色分道绳），每条泳道再均分 `kLaneSubdiv`=5 格得 0.5 m 的细线，
只有粗线标米数。**纵向零点在池边而不是画布边缘**：`configs/pool_mesh.json` 的
y 顶点是 `4.2358 / 4.7358 / 7.2358 … 22.2358 / 24.7358 / 25.2358`，中间 9 行正是
2.5 m 等距的 8 条泳道，首尾各多出 0.5 m 池岸。于是画布 21 m = 0.5 + **20** + 0.5，
网格纵向内缩 `kGridMarginM`=0.5 起算，左下角标 `50.0 x 20.0 m`（横向 50 m 是完整
池长，不内缩）。三个常数都在 `main.cpp`，改了标定就核那几个顶点。
实测粗线落在 y=50/300/550/…/2050，与量到的分道绳行 56/306/563/…/2045 逐条对上
（±16 px 是绳自身的弧垂）。
图内文字一律英文；泳道线本身挤到 4 px 以下才整个不画，细线单独降级（先只留泳道线）。
...
### 画面定向（`--rot180` / `--no-rot180`）

`--rot180` 把整块画布转 180°，**尺寸不变**（5002×2102 转完还是 5002×2102），
相机顺序、标定、LUT 一概不动。机位倒挂时用它，交付包的两个入口默认就带着它
（见 `scripts/dist.ps1`），现场追加 `--no-rot180` 即可转回正向 —— 两个是成对开关，
命令行里后写的生效。

**旋转做在帧源里，不是后处理**，两条输入路各自在「本来就要过一遍像素」的地方顺手
做掉，所以额外开销是零：

| 输入 | 怎么转 | 代价 |
| --- | --- | --- |
| `--cam-dir` | `stitch_kernel` 把落点改成 `(cw-1-x, ch-1-y)`，采样与累加一字不动 | 读写次数与地址跨度都不变，无额外遍数 |
| `--input` | 解码子进程加 `-vf hflip,vflip` | 与本进程的 GPU 推理天然并行，不占显存带宽 |

拼接那条路等价于「把 mesh 顶点绕画布中心转 180° 后重烘表」—— 该映射在整数网格上是
精确双射，不引入任何重采样。选改落点而不是真去重烘表，是为了让**定向与 LUT 解耦**：
一份表两种定向都能跑，换定向不必重烘、不必碰标定。实测两条路的首帧画布与
「不转的画布再 `[::-1,::-1]`」**逐字节相同**（`--dump-canvas` 对照，max diff 0）。

耗时也确认无变化：画布 3000 帧 17.6 → 17.7 ms/帧，六路现拼 26.4 → 26.5 ms/帧
（RTX 4080 Laptop，落在run 间抖动内）。

**但分析数字会小幅变化，这是预期的**：detect 对定向不是旋转等变的（letterbox 与
卷积都不是），同一段视频转与不转是**两组各自的基线**，不能相互 diff：

| 口径（RTX 4080 Laptop，3000 帧） | 人次 | track | 划水 |
| --- | --- | --- | --- |
| 画布 `--input` | 24107 | 63 | 875 |
| 画布 `--input --rot180` | 24233 | 90 | 859 |
| 六路现拼 `--cam-dir` | 25078 | 91 | 883 |
| 六路现拼 `--cam-dir --rot180` | 25578 | 119 | 883 |

划水与速度的算法本身**与定向无关**，不需要为旋转改一行：`elbow_angle` 是夹角，
`wrist_x_head` 的前进方向在运行期由首个可信的鼻子 x 现学（`metrics.cpp` 的 `s.dir`），
速度取框中心位移的模。上表的差异全部来自 detect 的框略有不同，而非指标口径变了。

`--input` 走 OpenCV 兜底（`--decoder cpu` 或 ffprobe 探测失败）时 `--rot180` 会
**直接报错**而不是悄悄退化：那条路没有滤镜链，转 180° 只能整帧再搬一次
（每帧 30 MB），与「零消耗」的前提相悖。

## 浏览器看板（`--web`）

第三条输出路，与 `--preview` / `--out` 并列：内嵌一个 HTTP 服务，把画面与分析结果
推给浏览器。**零第三方依赖**，只用系统 socket + OpenCV 的 JPEG 编码器。

```bash
# 默认 http://127.0.0.1:8080/，自动开浏览器
./build/swim_analyse --cam-dir configs/cameras.txt --models cpp/models --web --show-fps

# 换端口 / 给同网段看 / 不开浏览器（给了任一 --web* 参数即隐含 --web）
./build/swim_analyse --input data/xxx.mp4 --models cpp/models --web-port 9000
./build/swim_analyse --input data/xxx.mp4 --models cpp/models --web-bind 0.0.0.0
./build/swim_analyse --input data/xxx.mp4 --models cpp/models --web --no-browser
```

⚠️ **服务无认证、无 TLS**，所以默认只绑 `127.0.0.1`（本机可见）。
`--web-bind 0.0.0.0` 等于把实时泳池画面对同网段全部开放，只在自己可控的网络里用；
启动时会打一行实际监听地址，绑非回环时额外提示「已开在全部网口，同网段可见」。

Windows 双击 `scripts\web.bat`，第一个参数与 `preview.bat` / `analyse.bat` 完全同义
（省略 / `canvas` / `6cam` / 视频 / 目录 / 相机清单 / `rtsp://`）。交付包里是
`run_6cam_web.bat`。
...
| 路由 | 协议 | 内容 |
| --- | --- | --- |
| `/` | HTML | 单页前端，编译期嵌在 exe 里（`src/web_ui.cpp`） |
| `/meta` | JSON | 画布尺寸、`ppm`、`fps`、`kpt_thr`、骨架边表、网格常数。**一次性** |
| `/canvas.mjpg` | multipart/x-mixed-replace | 全景画布，**未标注**的原始像素 |
| `/follow.mjpg` | 同上 | 选中运动员的跟随裁切（纵向锁一条泳道，1225×350） |
| `/stats` | text/event-stream | 每帧一条 JSON：全场统计 + 逐人框/关键点 + 选中者明细 |
| `/select?id=N` | 204 | 选中 track，`-1` 取消 |

**为什么不用 WebSocket**：叠加层与统计都是「只要最新一份」的语义，SSE 天然满足且
浏览器端零解析代码；画面走 MJPEG 由 `<img>` 原生解码。于是省掉 SHA-1 握手、
base64、一整套帧协议。代价是画面与叠加层可能差一帧（约 16 ms），肉眼不可分。

**四个叠加层由浏览器重绘，不走 C++**。C++ 推的是未标注画面，前端按 `/meta` 的口径
用 `<canvas>` 画关键点 / 检测框 / 分析标签 / 泳池网格 —— 与窗口里按 1/2/3 是同一套口径
（`kGridMarginM` / `kLanePitchM` / `kLaneSubdiv` 都在 `/meta` 里），但**勾选框不重推流**，
切换零成本，且缩小的 JPEG 不会把框线一起缩糊。track 配色也同源：JS 的
`Math.imul(id, 2654435761)` 与 C++ 的 `id_rgb` 是同一个 Knuth 乘法散列。

**全景与跟随各有独立的一组勾选框**（关键点 / 检测 / 分析 / 网格），默认值按各自的用途给：

| | 关键点 | 检测（框） | 分析（标签） | 网格 |
| --- | --- | --- | --- | --- |
| 全景 | 开 | **关** | 开 | 开（全池，每 0.5 m 一刻度） |
| 跟随 | 关 | **关** | 开 | 开（**只画他这一道**，每 1 m 一刻度、每 5 m 标米数） |

跟随组还多一个**自动接管**（默认开，不是叠加层，见下）。

「检测」与「分析」刻意是**两个**开关：跟随视图要读速度与划数，但不要框线压在入水的手臂上。
跟随的分析标签**钉在窗口左上角的固定位置**，不跟着 bbox 上下抖；全景则必须贴各自的框沿
—— 同一条道可能两人并列，标签跟错人就没法看了。跟随的网格与米数换成橙黄并放大字号
（青色混在水色里读不出来），文字一律「黑描边 + 亮填充」，压在泳道线或水花上都还清楚。

**选中者在全景里靠骨架高亮认**：两组的检测框都默认关着（框线压在入水的手臂上反而挡
泳姿），所以「锁定的是谁」不能只靠框。选中者的骨架在暗底与彩色本体之间**多描一层白色
光环**并整体加粗，关键点再各加一圈暗描边（被加粗的亮线包着容易糊成一团）——
白色与框沿、标签边框的选中色同源，是同一套语言，开不开「检测」都成立。
他还被**排到最后画**：全景里人会重叠，压在别人骨架下面的高亮就白加了。
跟随视图里只画他一个（`solo`），不再加光环 —— 没有别人可比，加了只是糊。

页面两种布局，都能全屏：**全景 + 全场统计**、**全景 + 个人跟随 + 个人统计**。
前者把全场统计横过来放在**画布上方** —— 画布是 2.38:1 的扁形，右侧栏白吃宽度而上方
本来就用不满；后者高度才是紧的，仍走右侧栏。
在全景里点人即跟随（重叠时取面积最小的框），并自动切到第二种布局。

### 两套编号

**内部一律用原始 track id，界面一律显示「x 道 y 号」。** 前者是选人、跟随、所有统计
（速度 / 划水 / 里程 / 排名）的唯一键，全程不变、不出现在界面上；后者只服务显示，
会随换道重排，**拿它当键就会串**。分配在 `StatsBoard`（`take_slot`/`free_slot`），
随 `/stats` 下发（`p[].ln`/`p[].sl`、`sel.lane`/`sel.slot`）—— 占位表是有状态的，
前端逐帧重算不出同一个号。

道内号是**占位式**：每条道一张表，`occ_[lane][slot-1] = track id`，**下标即号码**，
`-1` 是空号。于是「补最小空号」就是找第一个 `-1`，不必另立号池：

| 情形 | 结果 |
| --- | --- |
| 新人入场，道内有空号 | 补**最小**那个（5 号退了，下一人就是 5 号） |
| 新人入场，道内号全被占 | 表尾加一个（10 人占满则下一人是 11 号） |
| 同一人穿行几条道 | 还旧道的号、在新道取号；**原始 track id 不变** |
| 短暂丢检（含 ghost 顶帧） | **保号 1.5 s**（`kSlotHoldSec`）—— 换号比号码空着更难认 |
| 人在池岸（`lane 0`） | 不给号，标签退化为「池岸」/ 渲染路的 `SIDE` |

换道判定带 **0.35 m 迟滞**（`kLaneHystM`）：要离开当前道，得越过分道绳再深入这么多。
压着绳游时 bbox 中心会在绳上来回，不设迟滞道号每帧跳、号也跟着乱换。
分道占用（`all.lanes`）也按这个带迟滞的道号数，与标签上的道号必然一致。
号的回收扫的是占位表而不是 `sum_` —— 后者从不回收、越跑越长，而占位表的规模就是
「此刻各道有几个人」。

**窗口与落盘 mp4 用同一个号**，只是 OpenCV 的 Hershey 字体没有中文，写作
`L7-1 S:3 1.01m/s`（池岸是 `SIDE`）。标签取自 `StatsBoard::display_no()`，
不重算，所以「窗口说 7 道 1 号、网页说 7 道 1 号」。

### 跟随镜头

裁切窗**纵向锁泳道、横向延迟跟随**，都在 `StatsBoard::track_camera()`（每帧一次）：

| 维度 | 规则 | 为什么 |
| --- | --- | --- |
| 高 | 一条泳道 + 上下各 0.5 m = 350 px，固定 | 泳姿让 bbox 忽大忽小，按框定高会不停变焦 |
| 宽 | 高 × 3.5 = 1225 px | 看板那一格很扁，按 16:9 裁会剩大片黑边 |
| 纵向中心 | 所在泳道中心线（池岸时退回人的 y），过同一条低通 | 换道时是平移而非跳变 |
| 横向中心 | 一阶低通追人，`tau = 0.6 s` | 滤掉 bbox 抖动；略微落后反而更像转播机位 |

`alpha = 1 - exp(-dt/tau)` 按秒定手感，换帧率或掉帧都是同一条时间常数。
平滑状态在 `update()` 里推进而非 `follow_rect()` 里 —— 后者一帧要被调两次
（推跟随流 + 写进 `/stats`），放那儿状态会多走一步。选中的人本帧不在场则不给 `rect`，
前端据此显示「已离场」，跟随流停在最后一帧。

**自动接管**（跟随组第五个勾选框，默认开）：让跟随视图在「有人可跟」时永不空着。
落点一律是 `all.hotid`，即「近 5 秒均速最快」的在场者 —— 与概览里那一行是同一个数。
判据用均速而非瞬时速度：瞬时每帧换人，读数与镜头都会乱闪。

三种情形走同一段代码（`autoFollow()`），差别只在宽限：

| 情形 | 行为 |
| --- | --- |
| 刚进「全景 + 个人」，还没点过人 | **立刻**跟上 `hotid`（没有可丢的目标，无需宽限） |
| 正在跟的人离场 | 等 1.5 秒再换人 |
| 全场无人（开场前 / 清场后） | 什么都不做，也不清计时；有人入场并观察满 3 秒即接管 |

1.5 秒的宽限只给「已经在跟的人」：短暂丢检很常见（服务端会用 ghost 顶几帧），
一丢就跳会让画面乱蹦。宽限按 `/stats` 的流内秒数 `t` 计，暂停或掉帧时与墙钟分叉也
不会误判。清场那一档不需要额外分支：`hotid` 为 -1 时直接返回，等有人入场时
`t - lostAt` 早已超过宽限，于是当即接管。手工点人立即重置计时。
代价是开着它就点不成「谁都不跟」（点空水面会被立刻接管回来），要空着就取消勾选。
`#ftip` 的文案按 `hotid` / `now` 分三档（正在接管 / 正在挑选 / 全场无人），
不会在无人可跟时说「正在接管」。

### 统计口径

**全场只报「此刻在场的真检出者」**：在场人数、平均速度、平均划频、平均每划距离、
**近 5 秒均速最快者及其编号**、分道占用。
刻意**不报**「累计人次」（逐帧人数之和，没有物理意义）、「累计 track」「划水合计」
「全场累计里程」（都会被 ID 切换灌水）—— 前三个只在控制台的 `[Summary]` 留着当回归判据。

「最快」用**近 5 秒均速**而不是瞬时速度峰值：后者每帧换人，读数闪得没法看，
而自动接管又要拿它当判据。实现是每个 track 留 6 格「每秒一次的里程快照」
（环形，`Track::rf/rd/rn`），取最老那格与当下作差 —— 不留逐帧历史是因为 `sum_`
从不回收，长跑时每人一份长数组就是常驻内存。入场不足 3 秒不参与排名
（里程还没积起来，一两次关键点抖动就能排到第一），因此清场后第一个下水的人有约
3 秒空窗，那期间 `hotid` 仍是 -1。排名从 -1 起比而不是 0：均速恒 ≥ 0，从 0 起比会把
「在场但没游动」的人判成没有，于是有人泡在水里时自动接管找不到落点。
窗口长度由 `kRecentSec` 一处定，
随 `/stats` 的 `all.hotsec` 下发给前端做标题，改常量不必改前端。

个人面板的标题就是他的「x 道 y 号」，内容为划水次数 / 当前与平均速度 / 划频 /
在场时长 / 累计里程 / 峰值速度 / 每划距离 / **划水指数**（`vavg × dps`，泳界通用的
效率指标：同样速度下越大越省力）。标题已含道号，故不再单列「所在泳道」。
均速一律用「里程 ÷ 在场时长」而非瞬时速度的平均 —— 后者会被丢检期间的
空档抬高。里程按 0.2 s 采样累加，与速度同量级；更密只会把关键点抖动累成虚假里程。

### 线程模型与开销

accept 一个线程 + 每连接一个线程 + 每路 MJPEG 一个编码线程。
`publish_*` 只做一次 `memcpy` 就返回，**绝不阻塞调用方**（它跑在后处理线程上）：
中间是「只保留最新一份」的 `Latest<T>` 广播格，慢客户端只会丢帧，不会把背压顶回
流水线。缩放（5002→1280，`INTER_AREA` 约 4 ms）与 JPEG 编码（约 1.5 ms）都在编码
线程上，不占帧预算。

**没人看就不干活**：`wants_canvas()` / `wants_follow()` / `wants_stats()` 按观众数门控，
分别挡掉整帧 memcpy、跟随裁切、以及每帧那条 JSON 的序列化（十几人 × 17 个关键点）。
无人观看时编码线程停在条件变量上，不占 CPU。

统计与 `[Summary]` / `--json` 共用 `StatsBoard` 一份口径，所以**控制台看到的数字与
网页看到的不可能分叉**。实测 3000 帧画布路开 `--web` 后 `24107 人次 / 63 track /
875 次划水`，与纯分析基线逐项相同。

代价只有 `need_image=true` 带来的整帧 D2H（已被 `cudaEvent` 重叠到 0.01 ms）与
发布侧一次 31.5 MB memcpy，实测 17.3 ms/帧（57.7 fps）—— 与 `--preview` 同档。
...
**运行中就能定位掉帧**（`--show-fps`）：每秒那行末尾跟一段
`| 到帧 29.9 30.1 29.8 30.0 29.9 30.0 丢0 顶0 重连0` —— 前六个是**每路各自的到帧率**
（`FrameSource::status()`，差分统计，只有 `--cam-dir` 这条实现有），后三项是累计的
丢弃 / 整帧顶住 / 重连次数。端到端帧率掉下来时它当场把责任分清：
**某一路明显低于其余路** = 那台相机或它那段网络；**六路一起低** = 下游跟不上
（NVDEC 吞吐或推理）。从前这三项只在退出时打一次，波动发生在运行中却看不见。
...
一致性（`--input` 画布路）：24107 人次、63 track、875 次划水；**纯分析与渲染两种模式的
...
| 左右分侧 | 自由泳/仰泳左右分开计数后**相加**（左右手各算一次划水），其余泳姿双臂同步、左右取均值后统一计数（`--stroke-type`） |
...
**实测对齐幅度**（同一份 `--dump` 关键点喂两边，3000 帧 / 63 track；**这组数字测于
左右取 min 的旧口径**，改成相加后绝对值会翻一倍多，两侧同步改动故相对差不变，
未重测）：`elbow_angle` C++ 376 次 vs Python 379 次（−0.8%），逐 track 平均绝对差
0.52、最大 4；`wrist_x_head` C++ 234 vs Python 232（+0.9%），平均绝对差 0.13、最大 1。
